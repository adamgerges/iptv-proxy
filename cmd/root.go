 ...
 */
 ...
 var rootCmd = &cobra.Command{
	 ...
	 Run: func(cmd *cobra.Command, args []string) {
		 ...
		 if username != "" && password != "" {
			 log.Printf("[iptv-proxy] INFO: Using dynamic credentials for Xtream provider!")
		 }
		 ...
		 conf := &config.ProxyConfig{
			 ...
			 XtreamUser:           config.CredentialString(username),
			 XtreamPassword:       config.CredentialString(password),
 ...
 func init() {
	 ...
	 rootCmd.Flags().StringP("custom-id", "", "", `Custom anti-collision ID for each track "http://proxy.com/<custom-id>/..."`)
	 ...
	 rootCmd.Flags().String("xtream-base-url", "", "Xtream-code base url e.g(http://example.tv:8080)")
